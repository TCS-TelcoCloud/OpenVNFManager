...
from vnfsvc.common import exceptions as exception
...
from vnfsvc.openstack.common import systemd
...
class Request(webob.Request):
    ...
    def best_match_language(self):
        ...
        if not self.accept_language:
            return None
        all_languages = gettextutils.get_available_languages('vnfsvc')
        return self.accept_language.best_match(all_languages)

    @property
    def context(self):
        if 'vnfsvc.context' not in self.environ:
            self.environ['vnfsvc.context'] = context.get_admin_context()
        return self.environ['vnfsvc.context']
